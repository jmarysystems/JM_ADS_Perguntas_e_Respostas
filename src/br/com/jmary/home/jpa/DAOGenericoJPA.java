...
public class DAOGenericoJPA<Gene> {
    ...
    public DAOGenericoJPA( Gene gene2, EntityManager em2 ) { try{
        ...
        } catch( Exception e ){ JOPM JOPM = new JOPM( 2, "public DAOGenericoJPA( Gene gene2, EntityManager em2 ) { try{ \n"
                + e.getMessage() + "\n", "DAOGenericoJPA<Gene>" ); }
    }
    
    public Collection consulta( Class clazz, String sql ){  
        Collection emps = null;
        
        try{
            
            emps = em.createNativeQuery(sql).getResultList();
            
            ////for (Iterator i = emps.iterator(); i.hasNext();) {
                
             ////   gene = (Gene) i.next();
            ////}
        } catch( Exception e ){ JOPM JOPM = new JOPM( 2, "Collection consulta( Class clazz, String sql ), \n"
    ...
    public void inserir() { try {                   
        ...
        } catch( Exception e ){ JOPM JOPM = new JOPM( 2, "public void inserir() { try { \n"
                + e.getMessage() + "\n", "DAOGenericoJPA<Gene>" ); } 
    }
    ...
    public void excluir(){ /*new Thread() {   @Override public void run() {*/ try { Thread.sleep( 1 );                   
        ...
        } catch( Exception e ){ JOPM JOPM = new JOPM( 2, "public void excluir() { try { \n"
                + e.getMessage() + "\n", "DAOGenericoJPA<Gene>" ); } //} }.start();
    }
